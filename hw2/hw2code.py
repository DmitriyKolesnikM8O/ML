...
class DecisionTree:
    ...
    def _fit_node(self, sub_X, sub_y, node, current_depth=0):
        ...
        for feature in range(sub_X.shape[1]):
            ...
            if gini is not None and (gini_best is None or gini > gini_best):
                ...
                left_size = np.sum(current_split)
                right_size = len(sub_y) - left_size
                
                if self._min_samples_leaf is not None and (left_size < self._min_samples_leaf or right_size < self._min_samples_leaf):
                    continue
                ...
                if feature_type == "real":
                    ...
                    threshold_best = [cat for cat, mapped_val in categories_map.items() if mapped_val < threshold]
                else:
                    raise ValueError
    ...
    def _predict_node(self, x, node):
        ...
        feature_type = self._feature_types[feature_to_split]
        ...
        if feature_type == "real":
            ...
            if x[feature_to_split] in node["categories_split"]:
                return self._predict_node(x, node["left_child"])
            else:
                return self._predict_node(x, node["right_child"])
        else:
